...
public class DumpMenuKindMining {
	public static void main(String[] args) {
		DumpMenuKindMining dump = new DumpMenuKindMining();
		dump.doDump();
	}
	...
	public void doDump() {
		...
		try {
			...
			for (int i=0; i<len; i++) {
				...
				sqlstr = "select kindmenuid from instance where orderid='"+oid+"'";
				rs = stmt.executeQuery(sqlstr);
				int count=0;
				while (rs.next()) {
					String muid = rs.getString("KINDMENUID");
